...
if __name__ == '__main__':
    ...
    _self_url = os.getenv("CARD_URL", "http://localhost:10003")
    ...
    # ------------------------------------------------------------------
    # Step: Add AP3 middleware (receiver)
    # ------------------------------------------------------------------
    # Persist keys so you can restart without changing identity.
    ...
    # Receiver advertises a blacklist commitment and supports PSI.
    ...
    ap3 = AP3Middleware(
        ...
        # Dev-only: company_a advertises a loopback card URL. Remove in production —
        # the SSRF guard will otherwise refuse the initiator's localhost URL.
        allow_private_initiator_urls=True,
    )

    # Wrap the existing executor: AP3 envelopes bypass the LLM/text path.
